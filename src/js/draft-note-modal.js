...
$(function () {
    ...
    $('#saveTaskBtn').on('click', async function () {
        ...
        if (mode === "new") {
            const newTask = { id: Date.now(), summary, description, start, end, googleEventId: null };
            tasks.push(newTask);
            
            // Отримуємо ID від Google та оновлюємо нотатку
            try {
                const googleId = await syncNoteToCalendar(summary, description, start, end);
            ...
            const index = tasks.findIndex(task => task.id === id);
                if (index !== -1) {
                tasks[index] = { ...tasks[index], summary, description, start, end };
            }
        }
